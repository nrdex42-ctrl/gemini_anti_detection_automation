...
POST_DIRECT_COMPOSER_TIMEOUT_SECONDS=45
POST_BATCH_HOME_WAIT_UNTIL=commit
POST_BATCH_HOME_READY_TIMEOUT_MS=2000
POST_BATCH_HOME_NETWORK_IDLE_TIMEOUT_MS=0
...
POST_PROFILE_SWITCH_CONFIRM_TIMEOUT_MS=1200
POST_INITIAL_UI_CONFIRMATION_TIMEOUT_MS=3500
POST_VIDEO_NETWORK_CONFIRMATION_TIMEOUT_MS=3500
POST_COMPOSER_BUTTON_TIMEOUT_MS=5000
POST_COMPOSER_DIALOG_TIMEOUT_MS=6500
MEDIA_UPLOAD_READY_IMAGE_TIMEOUT=30
MEDIA_UPLOAD_READY_VIDEO_TIMEOUT=120
FB_AUTOMATION_ENABLE_PRIVATE_HTTP=false
POST_PARALLEL_BATCH_ENABLED=false
POST_ALLOW_PARALLEL_SAME_COOKIE=false
POST_PARALLEL_SAME_COOKIE_MAX_CONTEXTS=1
MAX_PARALLEL_PAGES=1
POST_PARALLEL_SAME_COOKIE_STAGGER_SECONDS=10
...
POST_PARALLEL_PAGES_PORTAL_VIDEO_TIMEOUT_SECONDS=120
FACEBOOK_MEDIA_UPLOAD_MAX_ZERO_NETWORK_ATTEMPTS=2
...
The production posting mode defaults to account-safe sequential posting for each Facebook cookie session. This avoids driving the same cookie from multiple browser contexts at once. If you deliberately need faster same-media batches, set `POST_PARALLEL_BATCH_ENABLED=true`, `POST_ALLOW_PARALLEL_SAME_COOKIE=true`, and raise `MAX_PARALLEL_PAGES`, but that increases the chance that Facebook will invalidate the session or require login again.
