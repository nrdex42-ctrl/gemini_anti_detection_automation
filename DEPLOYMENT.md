...
POST_PARALLEL_SAME_COOKIE_STAGGER_SECONDS=0
...
The default posting mode is accuracy-first one-by-one for pages of the same Facebook account. This avoids profile-switch/session collisions during page posting. To re-enable parallel later, set `POST_PARALLEL_BATCH_ENABLED=true`, `POST_ALLOW_PARALLEL_SAME_COOKIE=true`, and raise `MAX_PARALLEL_PAGES` / `POST_PARALLEL_SAME_COOKIE_MAX_CONTEXTS` together.
