...
class TelegramBotApp:
    ...
    async def account_proxy_url(self, account_id: str, owner_id: Optional[int] = None) -> str:
        account_id = str(account_id or "").strip()
        if not account_id:
            return ""
        if hasattr(self.storage, "get_account_proxy"):
            proxy_url = await asyncio.to_thread(self.storage.get_account_proxy, account_id, owner_id)
            return normalize_proxy_url(proxy_url)
        try:
            account = await asyncio.to_thread(self.storage.get_account, account_id, owner_id)
        except Exception:
            return ""
        ...
        if pages:
            ...
            if selection_message_id:
                ...
                if session:
                    ...
                    self.set_dashboard_session(chat_id, user_id, session)
                    await self.send_post_stage_controls(chat_id, user_id, message_id, session, "page_select")
            return
    ...
    async def send_multi_video_caption_cards(
        self,
        chat_id: int,
        reply_to_message_id: int,
        session: Dict[str, Any],
    ...
    async def validate_video_url_or_reply(self, chat_id: int, message_id: int, text: str, lang: str = "en") -> str:
        ...
        if not ok:
            await self.send_message(chat_id, value, message_id, reply_markup=post_stage_reply_markup("media_video_url", lang=lang))
            return ""
    ...
    async def refresh_open_page_selection_card(
        ...
        *,
        ...
        await self.send_message(
            ...
            fallback_message_id,
            reply_markup=post_stage_reply_markup("page_select", lang=lang),
        )
    ...
    async def show_post_type_card(
        ...
        await self.send_post_stage_controls(chat_id, user_id, message_id, session, "post_type")
    ...
    async def apply_video_mode(
        ...
        *,
        ...
        if mode == "single_upload":
            ...
            self.set_dashboard_session(chat_id, user_id, session)
            text = post_input_card("video", lang=lang)
            if edit:
                await self.edit_message(chat_id, message_id, text, reply_markup={"inline_keyboard": []})
                await self.send_post_stage_controls(chat_id, user_id, message_id, session, "media_video")
            else:
                await self.send_message(chat_id, text, message_id, reply_markup=post_stage_reply_markup("media_video", lang=lang))
            return

        if mode == "single_url":
            session["step"] = "media_video_url"
            self.set_dashboard_session(chat_id, user_id, session)
            text = self.single_video_url_input_card(lang=lang)
            if edit:
                await self.edit_message(chat_id, message_id, text, reply_markup={"inline_keyboard": []})
                await self.send_post_stage_controls(chat_id, user_id, message_id, session, "media_video_url")
            else:
                await self.send_message(chat_id, text, message_id, reply_markup=post_stage_reply_markup("media_video_url", lang=lang))
            return
        ...
        if mode == "multi_upload":
            ...
            self.set_dashboard_session(chat_id, user_id, session)
            text = self.multi_video_prompt(session)
            if edit:
                await self.edit_message(chat_id, message_id, text, reply_markup={"inline_keyboard": []})
                await self.send_post_stage_controls(chat_id, user_id, message_id, session, "multi_video_upload")
            else:
                await self.send_message(chat_id, text, message_id, reply_markup=post_stage_reply_markup("multi_video_upload", lang=lang))
            return
        ...
        if mode == "multi_url":
            ...
            session["multi_media_paths"] = []
            self.set_dashboard_session(chat_id, user_id, session)
            text = self.multi_video_url_prompt(session)
            if edit:
                await self.edit_message(chat_id, message_id, text, reply_markup={"inline_keyboard": []})
                await self.send_post_stage_controls(chat_id, user_id, message_id, session, "multi_video_url")
            else:
                await self.send_message(chat_id, text, message_id, reply_markup=post_stage_reply_markup("multi_video_url", lang=lang))
            return
    ...
    async def show_post_input_card(
        ...
        await self.send_post_stage_controls(chat_id, user_id, message_id, session, str(session.get("step") or "caption"))
    ...
    async def show_post_review(
        ...
        *,
        ...
        if edit:
            ...
            await self.send_message(chat_id, text, message_id, reply_markup=markup)
        await self.send_post_stage_controls(chat_id, user_id, message_id, session, "review")
    ...
    async def validate_facebook_cookie_session(
        self,
        cookie_header: str,
        account_id: str,
        lang: str = "en",
        ...
        cookie_payload = cookies_json(parse_cookies(parsed.cookie_header))
        timeout_seconds = _env_float("BOT_COOKIE_VALIDATION_TIMEOUT_SECONDS", 8.0, minimum=1.0)
        try:
            return await asyncio.wait_for(
                validate_facebook_session(cookie_payload, proxy_url=proxy_url)
                if proxy_url
                else validate_facebook_session(cookie_payload),
                timeout=timeout_seconds,
            )
        except asyncio.TimeoutError:
            elapsed = format_elapsed_seconds(timeout_seconds)
            logger.warning("Cookie validation timed out for %s after %s", account_id, elapsed)
            if lang == "ar":
                return False, f"انتهى وقت فحص الكوكيز بعد {elapsed}. تم إيقاف العملية قبل تحديث الصفحات."
            return False, f"Cookie validation timed out after {elapsed}. Stopped before refreshing pages."

    async def auto_check_and_refresh_pages(
        self,
        account_id: str,
        user_id: int,
        lang: str,
        ...
        try:
            ...
            return {"ok": False, "stage": "cookie", "icon": "🔴", "detail": detail, "pages": []}

        if progress_callback:
            await progress_callback(1, "جاري البحث عن الصفحات المتاحة..." if lang == "ar" else "Finding available pages...")
        ...
        try:
            ...
            return {"ok": True, "stage": "pages", "detail": "Facebook session is valid", "pages": pages}
        except Exception as exc:
            detail = compact_error(exc, 500)
            logger.warning("Automatic page discovery failed for %s: %s", account_id, detail)
            return {"ok": False, "stage": "pages", "detail": detail, "pages": []}
    ...
    async def open_post_pages_after_auto_prepare(
        ...
        result = await self.auto_check_and_refresh_pages(account_id, user_id, lang, update_progress)
    ...
    async def switch_active_account_after_auto_prepare(
        self,
        chat_id: int,
        message_id: int,
        user_id: int,
        ...
        *,
        ...
        lang = lang or await self.user_language(user_id)
        owner_scope = self.account_owner_scope(user_id)
        account = await asyncio.to_thread(self.storage.get_account, account_id, owner_scope)
        ...
        title = "تجهيز الحساب النشط..." if lang == "ar" else "Preparing active account..."
        initial_text = progress_card(
            title,
            0,
            3,
            f"جاري فحص كوكيز {display}..." if lang == "ar" else f"Checking {display} cookies...",
        ...
        if edit:
            progress_message_id = await self.edit_or_send_message(chat_id, message_id, initial_text)
        else:
            progress = await self.send_message(chat_id, initial_text, message_id)
            progress_message_id = int((progress.get("result") or {}).get("message_id") or 0)
        ...
        if result.get("ok"):
            ...
            prefix = (
                f"تم تغيير الحساب النشط إلى {display}. تم تحديث {len(pages)} صفحة متاحة."
                if lang == "ar"
                else f"Active account switched to {display}. Refreshed {len(pages)} available page(s)."
            )
        else:
            ...
            if str(result.get("stage") or "") == "cookie":
                ...
                prefix = (
                    f"تم تغيير الحساب النشط إلى {display}. فشل تحديث الصفحات: {detail}"
                    if lang == "ar"
                    else f"Active account switched to {display}. Page refresh failed: {detail}"
                )
        await self.show_dashboard(chat_id, progress_message_id or message_id, prefix=prefix, user_id=user_id)
    ...
    async def handle_dashboard_session(
        ...
        if action == "manage_accounts" and step == "proxy_input":
            ...
            try:
                proxy_url = normalize_proxy_url(raw_value)
            except Exception as exc:
                await self.send_message(
                    chat_id,
                    (
                        f"صيغة البروكسي غير صحيحة: {exc}\n\n"
                        "ابعت البروكسي بصيغة http://user:pass@host:port أو socks5://host:port."
                        if lang == "ar"
                        else f"Invalid proxy format: {exc}\n\n"
        ...
        if action == "post" and step == "post_type_inline":
            ...
            if not post_type:
                await self.send_message(
                    ...
                    message_id,
                    reply_markup=post_stage_reply_markup("post_type", lang=lang),
                )
                return True
            session["post_type"] = post_type
            if post_type == "video":
            ...
            await self.send_message(
                chat_id,
                post_input_card(post_type, lang=lang),
                message_id,
                reply_markup=post_stage_reply_markup(str(session["step"]), lang=lang),
            )
            return True

        if action == "post" and step == "video_mode":
            mode = parse_video_mode_choice(text)
            if not mode:
                await self.send_message(
                    chat_id,
                    "استخدم أزرار طريقة الريلز." if lang == "ar" else "Use the video mode buttons.",
                    message_id,
                    reply_markup=post_stage_reply_markup("video_mode", lang=lang),
                )
                return True
        ...
        if action == "post" and step == "post_type_after_page":
            ...
            await self.send_message(
                chat_id,
                prompt_text("post", session["step"], lang=lang),
                message_id,
                reply_markup=post_stage_reply_markup(str(session["step"]), lang=lang),
            )
            return True
        ...
        if action == "post" and step == "caption":
            if not text.strip():
                await self.send_message(
                    ...
                    message_id,
                    reply_markup=post_stage_reply_markup("caption", lang=lang),
                )
                return True
        ...
        if action == "post" and step == "caption_after_media":
            ...
            if clean_text in {"skip", "none", "⏭️ skip", "⏭️ تخطي", "تخطي"}:
                ...
                session["caption"] = text if text.strip() else ""
            session["step"] = "review"
            self.set_dashboard_session(chat_id, user_id, session)
            await self.show_post_review(chat_id, message_id, user_id, session)
            return True
        ...
        if action == "post_all_pages" and step in {"media_image_all", "media_video_all"}:
            ...
            if not file_id:
                await self.send_message(chat_id, prompt_text("post", step, lang=lang), message_id, reply_markup=post_stage_reply_markup(step, lang=lang))
                return True
            pages = await asyncio.to_thread(
                self.storage.list_pages,
                str(session.get("account_id") or ""),
        ...
        if action == "post" and step == "multi_video_upload":
            ...
            if not file_id:
                await self.send_message(chat_id, self.multi_video_prompt(session), message_id, reply_markup=post_stage_reply_markup("multi_video_upload", lang=lang))
                return True
            selected_pages = self.selected_pages_from_session(session)
            if not selected_pages:
                self.clear_dashboard_session(chat_id, user_id)
            ...
            try:
                ...
                await self.send_message(
                    ...
                    message_id,
                    reply_markup=post_stage_reply_markup("multi_video_upload", lang=lang),
                )
                return True
        ...
        if action == "post" and step == "multi_video_url":
            ...
            if len(paths) < len(selected_pages):
                ...
                await self.send_message(
                    ...
                    message_id,
                    reply_markup=post_stage_reply_markup("multi_video_url", lang=lang),
                )
                return True
            session["step"] = "multi_caption"
            session["caption"] = ""
        ...
        if action == "post" and step in {"media_image", "media_video"}:
            ...
            attached_caption = str(message.get("caption") or "")
            if attached_caption.strip():
                ...
                await self.send_message(chat_id, prompt, message_id, reply_markup=post_stage_reply_markup("caption_after_media", lang=lang))
            return True
    ...
    async def handle_account_selected(
        ...
        *,
        ...
        if action in {"switch_account", "manage_accounts"}:
            await self.switch_active_account_after_auto_prepare(chat_id, message_id, user_id, account_id, edit=edit, lang=lang)
            return
    ...
    async def handle_callback_query(self, update: Dict[str, Any]) -> None:
        ...
        if data == "post:edit_caption":
            ...
            await self.send_post_stage_controls(chat_id, user_id, message_id, session, "caption_edit")
            return
    ...
    async def handle_update(self, update: Dict[str, Any]) -> None:
        ...
        if session and action not in explicit_escape_actions:
            if await self.handle_dashboard_session(chat_id, user_id, message_id, raw_text, message):
                return
        if action:
    ...
    async def discover_pages(self, account_id: str, owner_id: Optional[int] = None) -> List[Dict[str, str]]:
        ...
        try:
            ...
            ok, pages, detail = await discover_facebook_pages(
                cookies_json(parse_cookies(cookie_string)),
                proxy_url=proxy_url,
            )
            if ok and pages:
